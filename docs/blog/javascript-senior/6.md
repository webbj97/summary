...
```js
[1, '2', {}, null, true. undefined ].map((item) => typeof item);
// 返回
[
    "number",
    "string",
...
有一些意外，`typeof null => 'object'` 且 `typeof function(){} => 'function'`，看看下面的结果，你也意识到typeof也不是万能的：
...
> constructor 返回创建实例对象的 `Object` 构造函数的引用。注意，此属性的值是对函数本身的引用，而不是一个包含函数名称的字符串。
...
> Object.prototype.toString ，早期类型判断的方式
...
```js
[1, '2', true, null, undefined, function aaa(){}, [], {}].map((v) => Object.prototype.toString.call(v));
...
1. 大多数情况下我们可以使用`typeof`来检测基本数据类型，其他的都返回对应类型名的小写字母。
2. instanceof借助了原型链来判断，只要一个类型`Type`的prototype在一个`对象obj`的原型链上，那么`obj instanceof Type`就是true，否则就是false。
3. 对象的constructor会返回它的类型，而类型在定义的时候，会创建一个name只读属性，值为类型的名字。
4. stringTag 仍然可以返回较完整的类型判断
