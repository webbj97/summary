...
正如其字面意思，按需加载就是等到需要的时候才加载一部分模块。并不选择将其代码打包到首次加载的入口`bundle`中，而是等待触发的时机，届时才通过动态脚本插入的方式进行加载: 即创建`script`元素，添加脚本链接并通过appendChild加入到html元素中
...
1. 使用丰富且方便的JS新特性，如用 ES6，typescript
2. 使用 Css 预编译由，编写阅读性和扩展性良好的样式代码。
3. 提供开发时 SourceMap 功能，也即提供生产代码(如ES5)到源代码(typescript)的映射，方便开发调试
4. 提供生产时代码压缩功能，压缩js和css，删除注释，替换变量名(长变短)，减少代码加载体积
5. 其他.....

其实这些都是我们在工程项目中常见的需要，自动化构建工具的代表性工具有三个，分别是：

* 2012年出现的webpack
* 2012年出现的grunt
* 2013年出现的gulp

不过随着时间的发展，webpack 逐渐占据了主流，之后又有了 vite

每一个规范、工具的诞生和成熟，都离不开“问题”，不断地解决问题，也就不断的遇到新的问题。我们下一篇再见～